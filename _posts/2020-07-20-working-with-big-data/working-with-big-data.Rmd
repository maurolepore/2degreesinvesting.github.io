...
Processing time increases with increasing number of chunks; but large chunks may need more than your available memory and crash your R session.
...
For example, I tested `match_name()` with "big data", which size on disk as a .csv file was 20MB for the `loanbook` dataset and 100MB for the `ald` dataset. Feeding `match_name()` with those files directly crashed my R session. But feeding it with a `loanbook` dataset in 30 chunks run successfully in about 25'; the result had over 10 million rows:
...
The `loan_size_*` values are comparable across rows because they are all expressed in the same currency -- EURO:
